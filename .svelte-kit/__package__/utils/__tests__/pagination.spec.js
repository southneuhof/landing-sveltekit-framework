...
describe('withPagination', () => {
    ...
    it('rejects numeric strings', async () => {
        await expect(withPagination(async () => ({ data: [], total: 0 }), { page: '2', limit: '10' }))
    ...
    it('ignores perPage and keeps default limit', async () => {
        const result = await withPagination(async (_skip, take) => ({ data: [], total: 0 }), { page: 1, perPage: 1 });
        expect(result.meta.limit).toBe(10);
    });
    it('rejects array loader results', async () => {
        await expect(withPagination(async () => ['a', 'b'])).rejects.toThrowError(new TypeError('Pagination loader must return { data, total }'));
    });
    it('rejects invalid loader objects', async () => {
        await expect(withPagination(async () => ({ data: ['a'] }))).rejects.toThrowError(new TypeError('Pagination loader must return { data, total }'));
        await expect(withPagination(async () => ({ data: 'not-array', total: 1 }))).rejects.toThrowError(new TypeError('Pagination loader must return { data, total }'));
        await expect(withPagination(async () => ({ data: [], total: Number.NaN }))).rejects.toThrowError(new TypeError('Pagination loader must return { data, total }'));
    });
});
