export async function reorderEntries(options) {
    ...
    if (from < 1)
        throw new Error('Reorder "from" must be greater than or equal to 1');
    if (to < 1)
        throw new Error('Reorder "to" must be greater than or equal to 1');
    if (!delegate) {
    ...
    const axisWhere = options.axis.length === 1 && options.axis[0] === '*'
        ? {}
        : Object.fromEntries(options.axis.map((field) => {
            if (!(field in currentItem))
                throw new Error(`Axis field "${field}" not found on record`);
