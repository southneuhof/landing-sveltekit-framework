...
import { createFileManager, createImageManifestHandler, createLocalFileStorageDriver, createStorageUrlLocationStrategy, imageProcessor, resolveLocalStoragePath, } from '../index.js';
let tempRoot = null;
afterEach(async () => {
    if (tempRoot)
...
describe('file manager', () => {
    ...
    it('promotes nested temp URLs and deletes replaced files', async () => {
        ...
        const result = await manager.processPayloadFiles({ media: '/storage/temp/private/new.txt' }, {
    ...
    it('processes images, stores metadata, and exposes a manifest', async () => {
        ...
        expect((record?.data.variants).length).toBeGreaterThan(0);
        const manifestHandler = createImageManifestHandler({
            metadataStore,
            keyFromRequest: () => 'private/photo.jpg',
        });
        const response = await manifestHandler({ params: {}, url: new URL('http://localhost') });
        expect(response.status).toBe(200);
