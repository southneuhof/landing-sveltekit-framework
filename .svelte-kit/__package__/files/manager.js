...
export function createFileManager(config) {
    ...
    async function promoteTempFile(url) {
        const source = config.locations.fromUrl(url);
        if (!source || !config.locations.isTemp(source))
            return url;
    ...
    async function processPayloadFiles(input, options = {}) {
        const output = await processValue(input, options.previousData, options.deleteReplacedFiles ?? false);
        return output;
    }
    async function processValue(value, previousValue, deleteReplacedFiles) {
    ...
    function collectFileUrls(input) {
        const urls = new Set();
        function visit(value) {
            if (!value)
                return;
            if (typeof value === 'string') {
                if (config.locations.fromUrl(value))
                    urls.add(value);
                return;
            }
            if (Array.isArray(value)) {
    ...
    function createUploadHandler() {
        return async function POST({ request, params, url }) {
            ...
            const visibility = String(params.destination ?? config.upload?.defaultVisibility ?? 'private');
    ...
    async function findProcessor(file, bytes) {
        for (const processor of processors) {
            if (await processor.supports({ file, bytes }))
                return processor;
        }
        return null;
    }
    async function deleteDerivatives(metadata) {
        for (const derivative of metadata.derivatives ?? []) {
            await config.storage.delete(derivative);
        }
    }
    async function deleteBestEffort(url) {
        try {
    ...
    function fileFromServeEvent(event) {
        const params = event.params ?? {};
        const path = [params.storagePath, params.filename].filter(Boolean).join('/');
        if (path)
            return config.locations.fromUrl(`/storage/${path}`);
