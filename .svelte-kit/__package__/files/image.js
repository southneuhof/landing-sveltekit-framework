...
export function isPassthroughFile(mimeType) {
    return PASSTHROUGH_MIME_TYPES.has(mimeType.toLowerCase());
}
export function imageProcessor(config = {}) {
...
async function processImage(bytes, file, manager, config) {
...
async function encodeOriginal(bytes, file, quality) {
...
async function generatePlaceholder(bytes, config) {
...
async function generateVariants(bytes, file, manager, originalWidth, config) {
    const variants = [];
