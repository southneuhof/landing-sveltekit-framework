...
type CallReorderHandlerOptions = {
  body: any;
  modelConfig: any;
  prisma: any;
  locals?: any;
...
async function callReorderHandler({
  body,
  modelConfig,
  prisma,
  locals = {
...
function createPrisma() {
  ...
  const prisma = {
    ...
    $transaction: vi.fn(async (callback: any) => callback({ section: txDelegate })),
  };
    ...
    const response = await callReorderHandler({
      ...
      modelConfig: {
        ...
        reorder: {
          ...
          axis: ['section_group_id'],
          lifecycle: {
            post: async (_body: any, data: any) => ({ ...data, postProcessed: true }),
          },
        },
      },
    });
