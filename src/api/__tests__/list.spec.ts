...
async function callListHandler({
  url = 'http://localhost/api/article/list?page=2&limit=5',
      ...
      modelConfig: {
        ...
        list: {
          ...
          lifecycle: {
            ...
            post: async (data: any[]) => data.map((item) => ({ ...item, postProcessed: true })),
          },
        },
      },
    });
    ...
    const response = await callListHandler({
      ...
      modelConfig: {
        ...
        list: {
          ...
          lifecycle: {
            main: async () => ({ data: [{ id: 'a' }], total: 1 }),
            post: async () => ({ id: 'not-array' }) as any,
          },
        },
      },
    });
      ...
      modelConfig: {
        ...
        view: {
          customFields: [
            {
              name: 'label',
              generator: (data: any) => `Article ${data.id}`,
            },
          ],
        },
        list: {
