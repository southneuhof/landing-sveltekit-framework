...
function assertPaginationLoaderResult<T>(
  value: unknown,
): asserts value is PaginationLoaderResult<T> {
...
export async function withPagination<T>(
  loader: (skip: number, take: number) => Promise<PaginationLoaderResult<T>>,
  ...
  assertPaginationLoaderResult<T>(loaderResult);
