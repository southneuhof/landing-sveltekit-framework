...
describe('parseSearchParams', () => {
  it('throws for non-URLSearchParams inputs', () => {
    expect(() => parseSearchParams('page=1' as any)).toThrowError(
      new TypeError('parseSearchParams expects URLSearchParams'),
    );
    expect(() => parseSearchParams(null as any)).toThrowError(
      new TypeError('parseSearchParams expects URLSearchParams'),
    );
    expect(() => parseSearchParams(undefined as any)).toThrowError(
      new TypeError('parseSearchParams expects URLSearchParams'),
    );
  });
...
describe('validateFields', () => {
  ...
  it('passes full body as second argument', async () => {
    const validator = vi.fn((value: unknown, body: Record<string, unknown>) => {
  ...
  it('throws TypeError for legacy required shorthand', async () => {
    await expect(validateFields({ name: '' }, { name: { required: true } as any })).rejects.toThrowError(
      new TypeError('Validation rules for "name" must be an array'),
    );
  });

  it('throws TypeError for bare function validator', async () => {
    await expect(validateFields({ name: 'Ada' }, { name: (() => true) as any })).rejects.toThrowError(
      new TypeError('Validation rules for "name" must be an array'),
    );
  });

  it('throws TypeError for rule object without validator', async () => {
    await expect(validateFields({ name: '' }, { name: [{ message: 'Name is required' } as any] })).rejects.toThrowError(
      new TypeError('Validation rule for "name" must define a validator'),
    );
  });
});
...
describe('transformFieldsForeign', () => {
  ...
  it('throws for array shorthand', () => {
    expect(() => transformFieldsForeign({ role: ['id', 'name'] as any })).toThrowError(
      new TypeError('Invalid fieldsForeign config for "role"'),
    );
  });

  it('throws for raw Prisma passthrough', () => {
    expect(() => transformFieldsForeign({ role: { select: { id: true } } as any })).toThrowError(
      new TypeError('Invalid fieldsForeign config for "role"'),
    );
  });

  it('throws for empty relation config', () => {
    expect(() => transformFieldsForeign({ role: {} })).toThrowError(
      new TypeError('Invalid fieldsForeign config for "role"'),
    );
  });

  it('throws for non-array fields', () => {
    expect(() => transformFieldsForeign({ role: { fields: 'id' as any } })).toThrowError(
      new TypeError('Invalid fieldsForeign config for "role"'),
    );
  });

  it('throws for unknown keys', () => {
    expect(() => transformFieldsForeign({ role: { fields: ['id'], include: {} } as any })).toThrowError(
      new TypeError('Invalid fieldsForeign config for "role"'),
    );
  });
});
