...
export type SectionRenderConfig = {
    ...
    resolveWrapper?: (input: {
        section: LandingSection;
    }) => SectionWrapperRenderConfig;
};
...
type SlotField<TSlot extends SectionSchemaSlot> = TSlot['fields'] extends readonly (infer TField)[] ? TField extends string ? TField : never : never;
...
type NestedLandingSectionForSchema<TSchema extends NestedSectionSchema> = Omit<LandingSection, 'meta' | 'data'> & {
    meta: TSchema['meta'] extends SectionSchemaMeta ? Partial<Record<Extract<TSchema['meta']['fields'] extends readonly (infer TField)[] ? TField : never, string>, any>> : Record<string, any>;
    data: {
        [K in Extract<keyof TSchema['data'], string>]: SectionDataBySlot<TSchema['data'][K]>;
    };
};
type SectionDataBySlot<TSlot extends SectionSchemaSlot> = TSlot['type'] extends 'resource' ? TSlot['many'] extends true ? AnyRecord[] : AnyRecord | null : TSlot['type'] extends 'content' | 'gallery' ? TSlot['many'] extends true ? SlotDataValue<TSlot>[] : SlotDataValue<TSlot> : TSlot['type'] extends 'section' | 'sectionGroup' ? TSlot['schema'] extends NestedSectionSchema ? TSlot['many'] extends true ? Array<NestedLandingSectionForSchema<TSlot['schema']>> : NestedLandingSectionForSchema<TSlot['schema']> : TSlot['many'] extends true ? LandingSection[] : LandingSection : unknown;
